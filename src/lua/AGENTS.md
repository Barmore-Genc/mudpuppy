...
- `mod.rs`: `LuaEngine` (sandbox setup, `core.luau` + user-config load/hot-reload, key dispatch, event firing), `EventKind`, and `config_path` (hand-rolled XDG resolver).
...
- `core.luau`: the embedded default keymap, expressed against the primitive verbs; `include_str!` in release, read from disk in debug so it hot-reloads in dev.
