...
- `state.rs`: `StateFile` (versioned store root, incl. `log_seed` debug-log salt rotated by `clear`), `Target`, `Turn` (turn-protocol block), `SCHEMA_VERSION`; upsert/remove/clear/merge-by-id helpers.
