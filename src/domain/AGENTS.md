...
- `annotation.rs`: the `Annotation` type and its enums (`Author`, `Severity`, `Tag`, `Status`, `Side`, `AnchorScope`); `end_line` (region) + `scope` (line/whole-file) fields, `Annotation::new_id`, CLI string parsing (`FromStr`) and `ParseEnumError`.
