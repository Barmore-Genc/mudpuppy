...
- `update.rs`: self-update — `gh`-based GitHub release check, semver comparison, and version-validated (`vX.Y.Z` only) `cargo install`; surfaced to Lua as `mudpuppy.updates` and driven by `core.luau`'s update prompt.
