...
- `update.rs`: self-update (behind the off-by-default `auto-update` feature) — release check by HTTPS GET of the GitHub Pages `dist-manifest.json` (no `gh` needed), semver comparison, the release changelog, and install via `axoupdater` (downloads + checksum-verifies the prebuilt binary); surfaced to Lua as `mudpuppy.updates` and driven by `core.luau`'s update prompt.
